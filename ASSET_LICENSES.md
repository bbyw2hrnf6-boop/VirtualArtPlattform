...
The house geometry, metric joinery, furniture, procedural surface shaders, beech canopies, Cycles renders and browser derivatives are authored for this project from that dimensional brief. The realism refinement adds scanned materials and vegetation from **Poly Haven, CC0-1.0**: `rock_08`, `oak_veneer_01`, `forest_ground_04`, `leafy_grass`, `bark_brown_02`, `mossy_rock`, `fern_02`, `periwinkle_plant` and `tree_small_02`. Author credits, exact download URLs, original checksums and SHA-256 hashes are retained in [the material source manifest](blender/showcases/forest-fold-house/materials/sources.json). Source files are retained alongside that manifest. Licence: [Poly Haven asset licence](https://polyhaven.com/license). The scanned stone is an appearance reference for the concept's gneiss finish, not a geological identification.

No external HDRI or purchased furniture is used. Public WebP images are encoding derivatives of actual Blender output; the original AI reference pictures do not ship as runtime backdrops or pretend 3D previews. The concept is not a construction document or a claim of a built property.
