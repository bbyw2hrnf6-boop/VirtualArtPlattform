...
for (const [name, viewport] of [
  ...
  test(`guest publish is explicit, profile-free and recoverable on ${name}`, async ({ page }) => {
    ...
    await expect(review.getByText(/request up to 7 days in Explore.*listing starts only if approved/i)).toBeVisible();
    await expect(review.getByText("Request Creator Hub placement", { exact: true })).toHaveCount(0);
    ...
    await expect(page.getByText(/Guest Space; no profile\..*Explore pending quality\/safety review/)).toBeVisible();
    await expect(page.getByText(/Link live until/)).toBeVisible();
