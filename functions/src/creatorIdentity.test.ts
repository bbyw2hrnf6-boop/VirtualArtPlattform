...
describe("Creator identity contract", () => {
  ...
  it("lists only public profiles through the Creator directory projection", () => {
    ...
    expect(publicCreatorDirectoryEntry({ ...profile, profilePublic: true, discoverEligible: false })).toEqual({
      handle: "studio-north",
      displayName: "Studio North",
      bio: "Spatial work.",
      imagePresent: true,
  ...
  it("uses the owner's public/private choice as the sole visibility gate", () => {
    const publicProfile = parseCreatorProfileInput({
      ...
      profilePublic: true, discoverEligible: false,
    });
    expect(isPublicCreatorProfile(publicProfile)).toBe(true);
    const privateProfile = parseCreatorProfileInput({
      handle: "studio-north", displayName: "Studio North", bio: "Spatial work.", links: [],
      profilePublic: false, discoverEligible: true,
    });
    expect(isPublicCreatorProfile(privateProfile)).toBe(false);
    expect(publicProfile).not.toHaveProperty("discoverEligible");
  });
