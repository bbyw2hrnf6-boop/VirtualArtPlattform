...
export function verificationMail(
  ...
  const subject = "One click. Your AURA space is ready.";
  const text = `Hi ${name},\n\nVerify your email to secure your AURA rooms and use account preview access.\n\nVerify email: ${input.verificationUrl}\n\nAURA is currently a free preview. Paid plans and additional professional tools are planned, but billing is not active.\n\nData & rights: ${brand.appUrl}/#/data\nContact: ${brand.replyTo}\n${brand.legalFooter}`;
  const body = `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-top:1px solid #c9c9c1;border-bottom:1px solid #c9c9c1"><tr><td style="padding:20px 0"><strong style="font-family:Georgia,serif;font-size:22px;font-weight:400">Why verify?</strong><p style="margin:9px 0 0;color:#676960;font-size:12px;line-height:1.7">Keep rooms attached to one identity, publish public, unlisted, or private previews, and return to update the same live room later.</p></td></tr></table>`;
  return {
    subject,
    text,
    html: emailShell({
      brand,
      preheader: "Verify your email and return to your AURA rooms.",
      eyebrow: `Welcome, ${name}`,
      title: "Your space starts here.",
      intro: "Confirm this email address to keep control of your rooms and unlock account preview access.",
      body,
      actionLabel: "Verify email",
      actionUrl: input.verificationUrl,
      footerAction: `This transactional email was requested for an AURA account. Data & rights: <a href="${escapeHtml(brand.appUrl)}/#/data" style="color:#4f514a">open notice</a>.`,
    }),
  };
}
