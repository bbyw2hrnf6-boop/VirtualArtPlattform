...
function HeroGalleryVisual() {
  return <button className="hero-gallery-visual" onClick={() => navigate('/demo')} aria-label="Enter the Danny Hirsch live gallery"><img src="./assets/demo/aura-hero-gallery.webp" width="1600" height="963" fetchPriority="high" decoding="async" alt="Atmospheric AURA gallery with contemporary artworks"/><span className="hero-gallery-shade"/><span className="hero-gallery-label"><i>Live space · 01</i><strong>Enter the exhibition ↗</strong></span></button>;
}
    ...
    <div className={`discover-grid ${!galleries.length ? 'discover-grid--empty' : ''}`}>{!galleries.length && <div className="discover-empty"><span>{status === 'loading' ? 'Looking for open exhibitions…' : status === 'error' ? 'Discover is temporarily unavailable.' : 'No exhibitions are open yet.'}</span><p>{status === 'error' ? 'Your published gallery is safe. Please retry the connection.' : 'Publish the first gallery and it will appear here for ten days.'}</p><button className="text-link" onClick={status === 'error' ? load : () => navigate('/create')}>{status === 'error' ? 'Try again →' : 'Create a gallery →'}</button></div>}{visibleGalleries.map((gallery) => {
      ...
      return <article key={gallery.id} className={`discover-card template-card--${gallery.templateId}`}>
        <button className="discover-card-main" onClick={() => navigate(`/g/${gallery.id}`)}>
          <div className="discover-cover">{cover ? <img src={cover} alt="" loading="lazy" decoding="async"/> : <div className="mini-room"><i/><i/><i/></div>}<span>{days} days left</span></div>
...
function Landing() {
  return <main className="landing"><Header />
    ...
    <DiscoverGalleries />
    <section className="promise"><p className="eyebrow">Quality over quantity</p><div><h2>From studio to<br/><em>space</em> in minutes.</h2><p>A small collection of carefully designed, Blender-based environments gives artists cinematic light and premium materials without the complexity of 3D software.</p></div><div className="steps">{['Choose a gallery','Upload artwork','Customize','Publish & share'].map((step, i) => <article key={step}><span>0{i+1}</span><h3>{step}</h3><p>{['Begin with one of three considered, photorealistic spaces.','Add up to fourteen works and compose them directly on the walls.','Set the mood with curated finishes, lighting, and objects.','Open your exhibition and share one simple link with the world.'][i]}</p></article>)}</div></section>
...
function initialArtworkPlacement(templateId: TemplateId, slot: number): Pick<Artwork, 'wall' | 'x' | 'y' | 'scale'> {
  if (templateId === 'pavilion') {
    const placements: Array<[WallId, number, number]> = [
      ['divider-front', -4, 2.55], ['divider-front', 0, 2.55], ['divider-front', 4, 2.55],
      ['divider-back', -4, 2.55], ['divider-back', 0, 2.55], ['divider-back', 4, 2.55],
      ['north', -11, 2.65], ['north', 0, 2.65], ['north', 11, 2.65],
      ['south', -11, 2.65], ['south', 0, 2.65], ['south', 11, 2.65],
      ['west', -18, 2.65], ['east', 18, 2.65]
    ];
...
const WALL_LABELS: Record<WallId, string> = {
  north: 'Back wall', south: 'Entrance wall', west: 'Left wall', east: 'Right wall',
  'divider-front': 'Feature wall A', 'divider-back': 'Feature wall B'
};
const PAVILION_WALL_LABELS: Record<WallId, string> = {
  north: 'North gallery', south: 'Entrance gallery', west: 'West wing', east: 'East wing',
  'divider-front': 'Feature wall A', 'divider-back': 'Feature wall B'
};
const wallLabel = (templateId: TemplateId, wall: WallId) => templateId === 'pavilion' ? PAVILION_WALL_LABELS[wall] : WALL_LABELS[wall];
...
function openArtworkPosition(draft: GalleryDraft, artworkId: string, wall: WallId, preferredX = 0, preferredY = 2.25) {
  const template = TEMPLATES.find((item) => item.id === draft.templateId) ?? TEMPLATES[0];
  const artwork = draft.artworks.find((item) => item.id === artworkId); if (!artwork) return null;
  const wallWidth = wall.startsWith('divider') ? (template.dividerWidth ?? 6.2) : wall === 'north' || wall === 'south' ? template.dimensions[0] : template.dimensions[1];
  const wallHeight = wall.startsWith('divider') ? template.height - .75 : template.height;
  const artworkHeight = 1.5 * artwork.scale; const artworkWidth = artworkHeight * artwork.aspect;
  const maxX = Math.max(.12, wallWidth / 2 - artworkWidth / 2 - .18); const minY = artworkHeight / 2 + .16; const maxY = Math.max(minY, wallHeight - artworkHeight / 2 - .14);
  const targetX = Math.max(-maxX, Math.min(maxX, preferredX)); const targetY = Math.max(minY, Math.min(maxY, preferredY));
  const candidates: Array<{ x: number; y: number; distance: number }> = [];
  for (let y = minY; y <= maxY + .001; y += .24) for (let x = -maxX; x <= maxX + .001; x += .24) candidates.push({ x, y, distance: (x - targetX) ** 2 + (y - targetY) ** 2 * 1.35 });
  candidates.push({ x: targetX, y: targetY, distance: -1 }); candidates.sort((a, b) => a.distance - b.distance);
  const others = draft.artworks.filter((item) => item.id !== artworkId && item.wall === wall);
  return candidates.find((candidate) => others.every((other) => {
    const otherHeight = 1.5 * other.scale; const otherWidth = otherHeight * other.aspect;
    return Math.abs(candidate.x - other.x) >= (artworkWidth + otherWidth) / 2 + .32 || Math.abs(candidate.y - other.y) >= (artworkHeight + otherHeight) / 2 + .22;
  })) ?? null;
}
...
function Studio({ initialTemplate }: { initialTemplate: TemplateId }) {
  ...
  const requestWallFocus = useCallback((wall: WallId) => setWallFocus({ wall, token: ++wallFocusToken.current }), []);
  const selectArtwork = useCallback((id: string) => { setSelectedId(id); setSelectedDecorId(undefined); }, []);
  const openArtwork = (id: string) => { const artwork = draft.artworks.find((item) => item.id === id); selectArtwork(id); setPlacementNotice(undefined); if (artwork) requestWallFocus(artwork.wall); };
  ...
  const changeArtworkWall = (wall: WallId) => {
    if (!selected) return;
    if (selected.wall === wall) { setPlacementNotice(`Viewing ${wallLabel(draft.templateId, wall)}. Click an empty point to place, then drag to refine.`); requestWallFocus(wall); return; }
    const placement = openArtworkPosition(draft, selected.id, wall, 0, selected.y);
    if (!placement) { setPlacementNotice(`${wallLabel(draft.templateId, wall)} is full. Choose another wall or reduce the artwork size.`); return; }
    updateArtwork({ wall, x: placement.x, y: placement.y }); setPlacementNotice(`Placed on ${wallLabel(draft.templateId, wall)}. Click the wall for an exact position, then drag to refine.`); requestWallFocus(wall);
  };
  ...
  const placeArtwork = useCallback((id: string, wall: WallId, x: number, y: number) => setDraft((current) => {
    const placement = openArtworkPosition(current, id, wall, x, y); if (!placement) return current;
    return { ...current, artworks: current.artworks.map((item) => item.id === id ? { ...item, wall, x: placement.x, y: placement.y } : item) };
  }), []);
  ...
  const addDecor = (type: DecorId) => {
    ...
    const item: DecorPlacement = { id: crypto.randomUUID(), type, x: decorInsertion.current.x, z: decorInsertion.current.z, rotation: 0, scale: 1 };
    update('decor', [...draft.decor, item]); setSelectedDecorId(item.id); setSelectedId(undefined);
  };
  const curateWithAi = async () => {
    ...
    try {
      const result = await autoCurateGallery(draft, roomTemplate, setCurationPhase);
      const curatedDraft = { ...result.draft, decor: result.draft.decor.slice(0, MAX_DECOR_OBJECTS) };
      setDraft(curatedDraft); setCurationReport({ ...result.report, decorCount: curatedDraft.decor.length });
    }
  ...
  const upload = async (files: FileList | null) => {
    ...
    try {
      for (const file of supported) { try { const image = await imageFromFile(file); const slot = draft.artworks.length + next.length; next.push({ id: crypto.randomUUID(), title: file.name.replace(/\.[^.]+$/, '').slice(0, 80), ...image, ...initialArtworkPlacement(draft.templateId, slot) }); } catch (error) { failures.push(error instanceof Error ? error.message : `${file.name} could not be prepared.`); } }
      ...
      if (next.length) { setDraft((current) => ({ ...current, artworks: [...current.artworks, ...next] })); setCurationReport(undefined); setCurationSnapshot(undefined); selectArtwork(next[0].id); requestWallFocus(next[0].wall); }
  ...
  const publish = async () => {
    ...
    catch (error) { console.error(error); setPublishError(error instanceof Error ? error.message : 'Publishing could not connect to Firebase. Please check the connection and try again.'); }
  ...
  return <main className="studio"><header className="studio-header"><Logo/><div className="studio-title"><input aria-label="Gallery title" maxLength={100} value={draft.title} onChange={(event) => update('title', event.target.value)}/><span>by</span><input aria-label="Artist name" maxLength={100} value={draft.artist} onChange={(event) => update('artist', event.target.value)}/></div><div className="studio-actions"><button className="ai-curate-button" onClick={() => void curateWithAi()} disabled={!draft.artworks.length || curating || uploading} title={draft.artworks.length ? 'Automatically curate this exhibition' : 'Upload artwork first'}><span>✦</span>{curating ? 'Curating…' : 'AI Curator'}</button><button className="publish-button" onClick={publish} disabled={publishing || uploading || curating}>{publishing ? 'Publishing…' : 'Publish'} <span>↗</span></button></div></header>{publishError && <div className="studio-alert" role="alert"><span>Before publishing</span>{publishError}<button onClick={() => setPublishError(undefined)} aria-label="Dismiss publishing message">×</button></div>}
    <div className="studio-body"><aside className="tool-panel"><section className="mobile-exhibition"><p className="tool-label">Exhibition details</p><label>Gallery title<input maxLength={100} value={draft.title} onChange={(event) => update('title', event.target.value)}/></label><label>Artist name<input maxLength={100} value={draft.artist} onChange={(event) => update('artist', event.target.value)}/></label></section><section><p className="tool-label">01 · Artwork</p><label className={`upload ${uploading ? 'is-uploading' : ''}`} aria-busy={uploading}><input className="visually-hidden" type="file" aria-label="Upload artwork images" accept="image/*,.heic,.heif" multiple disabled={uploading} onChange={(event) => { const input = event.currentTarget; void upload(input.files).finally(() => { input.value = ''; }); }}/><span aria-hidden="true">{uploading ? '◌' : '＋'}</span><strong>{uploading ? 'Preparing artwork…' : 'Upload artwork'}</strong><small>{uploading ? 'Optimizing for the gallery' : `JPG, PNG, WebP or HEIC · up to ${maxArtworks}`}</small></label>{uploadError && <p className="upload-error" role="alert">{uploadError}</p>}<div className="artwork-list">{draft.artworks.map((artwork, index) => <button key={artwork.id} className={selectedId === artwork.id ? 'active' : ''} aria-pressed={selectedId === artwork.id} onClick={() => openArtwork(artwork.id)}><img src={artwork.src} alt=""/><span>{String(index + 1).padStart(2,'0')} · {artwork.title}</span></button>)}</div>{selected && <div className="placement"><WallPicker templateId={draft.templateId} artworks={draft.artworks} activeWall={selected.wall} onChoose={changeArtworkWall}/><p className="placement-guide"><span><strong>Choose wall → click to place → drag to refine.</strong><br/>{placementNotice ?? 'The camera flies to the wall and finds a free position automatically.'}</span></p><label>Title<input type="text" value={selected.title} maxLength={80} onChange={(event) => updateArtwork({ title: event.target.value })}/></label><label>Year<input type="text" value={selected.year ?? ''} maxLength={12} placeholder="2026" onChange={(event) => updateArtwork({ year: event.target.value })}/></label><label className="placement-note">Artwork note<textarea value={selected.description ?? ''} maxLength={240} placeholder="A short note visitors can read…" onChange={(event) => updateArtwork({ description: event.target.value })}/></label><label>Fine placement<select value={selected.wall} onChange={(event) => changeArtworkWall(event.target.value as WallId)}><option value="north">Back wall</option><option value="south">Entrance wall · Behind you</option><option value="west">Left wall</option><option value="east">Right wall</option>{draft.templateId === 'pavilion' && <><option value="divider-front">Feature wall A</option><option value="divider-back">Feature wall B</option></>}</select></label><Range label="Horizontal" min={-artworkLimit} max={artworkLimit} step={.1} value={selected.x} onChange={(x) => updateArtwork({ x })}/><Range label="Height" min={1} max={Math.max(3.6, roomTemplate.height - (selected.wall.startsWith('divider') ? 1.25 : .75))} step={.1} value={selected.y} onChange={(y) => updateArtwork({ y })}/><Range label="Size" min={.45} max={1.65} step={.05} value={selected.scale} onChange={(scale) => updateArtwork({ scale })}/><button className="remove" onClick={() => { update('artworks', draft.artworks.filter((item) => item.id !== selectedId)); setSelectedId(undefined); }}>Remove artwork</button></div>}</section>
...
function Swatches({ options, value, onChange }: { options: [string,string,string?][]; value: string; onChange: (value: string) => void }) { return <div className="swatches">{options.map(([name,color,label]) => <button type="button" key={name} className={value === name ? 'active' : ''} aria-pressed={value === name} onClick={() => onChange(name)}><i style={{ background: color }}/><span>{label || name}</span></button>)}</div>; }
function Choice({ options, value, onChange }: { options: string[]; value: string; onChange: (value: string) => void }) { return <div className="choices">{options.map((item) => <button type="button" key={item} className={value === item ? 'active' : ''} aria-pressed={value === item} onClick={() => onChange(item)}>{item}</button>)}</div>; }
...
function ArtworkInfoCard({ artwork, onClose }: { artwork: ArtworkFocus; onClose: () => void }) {
  ...
  return <aside className="artwork-info" role="dialog" aria-labelledby={titleId}>{artwork.image && <img src={artwork.image} alt={artwork.title}/>}<div><p className="eyebrow">Selected artwork</p><button ref={closeButton} onClick={onClose} aria-label="Close artwork information">×</button><h2 id={titleId}>{artwork.title}</h2><span>{artwork.artist}{artwork.year ? ` · ${artwork.year}` : ''}</span><p>{artwork.description || 'Presented as part of this virtual exhibition.'}</p></div></aside>;
}

function MovementHint({ viewMode }: { viewMode: ViewMode }) {
  return <div className="movement-hint" role="note">
    <span className="movement-hint__desktop">{viewMode === 'walk' ? 'WASD to walk · ↑↓ move · ←→ turn · Click floor to move' : 'Drag to orbit · Scroll to zoom'}</span>
    <span className="movement-hint__mobile">{viewMode === 'walk' ? 'Drag to look · Tap floor to walk · Pinch to zoom' : 'Drag to orbit · Pinch to zoom'}</span>
  </div>;
}
...
function Demo() {
  ...
  return <main className="viewer"><header className="viewer-header"><Logo/><div><p>Danny Hirsch Arts</p><span>Threshold · 2026</span></div><button onClick={() => navigate('/create')}>Create your own ↗</button></header><DannyDemoScene viewMode={viewMode} playIntro onArtworkFocus={setArtworkFocus}/><ViewSwitch value={viewMode} onChange={changeView}/>{artworkFocus && <ArtworkInfoCard artwork={artworkFocus} onClose={() => setArtworkFocus(null)}/>}<div className="viewer-caption"><p className="eyebrow">Public demo gallery</p><h1>Threshold</h1><p>Material, movement, and atmosphere by Danny Hirsch.</p></div><MovementHint viewMode={viewMode}/></main>;
}

function PublishedGallery({ id }: { id: string }) {
  const [loadState, setLoadState] = useState<GalleryLoadState>({ status: 'loading' });
  const [loadAttempt, setLoadAttempt] = useState(0);
  const [viewMode, setViewMode] = useState<ViewMode>('walk');
  const [artworkFocus, setArtworkFocus] = useState<ArtworkFocus | null>(null);
  useEffect(() => {
    let stale = false;
    galleryRepository.find(id).then((gallery) => {
      if (!stale) setLoadState(gallery ? { status: 'ready', gallery } : { status: 'not-found' });
    }).catch((error) => {
  ...
  useEffect(() => {
    if (loadState.status === 'ready') document.title = `${loadState.gallery.title} — ${loadState.gallery.artist} | AURA`;
  }, [loadState]);
  if (loadState.status === 'loading') return <main className="loading" role="status" aria-live="polite">Loading space…</main>;
  if (loadState.status === 'error') return <main className="not-found not-found--error"><Logo/><p className="eyebrow">Connection interrupted</p><h1>We couldn't open this gallery.</h1><p>The exhibition may still be live. Check your connection and try again.</p><div className="not-found-actions"><button className="button button--light" onClick={() => { setLoadState({ status: 'loading' }); setLoadAttempt((attempt) => attempt + 1); }}>Try again</button><button className="text-link" onClick={() => navigate('/')}>Return home</button></div></main>;
  if (loadState.status === 'not-found') return <main className="not-found"><Logo/><h1>This gallery isn't available.</h1><p>The exhibition may have reached the end of its ten-day run.</p><button className="button button--light" onClick={() => navigate('/create')}>Create a gallery</button></main>;
  const gallery = loadState.gallery;
  const changeView = (value: ViewMode) => { setArtworkFocus(null); setViewMode(value); };
  return <main className="viewer"><header className="viewer-header"><Logo/><div><p>{gallery.title}</p><span>{gallery.artist}</span></div><button onClick={() => navigate('/create')}>Create your own ↗</button></header><GalleryScene draft={gallery} visitor viewMode={viewMode} playIntro onArtworkFocus={setArtworkFocus}/><ViewSwitch value={viewMode} onChange={changeView}/>{artworkFocus && <ArtworkInfoCard artwork={artworkFocus} onClose={() => setArtworkFocus(null)}/>}<div className="viewer-caption"><p className="eyebrow">Virtual exhibition</p><h1>{gallery.title}</h1><p>by {gallery.artist}</p></div><MovementHint viewMode={viewMode}/></main>;
}
...
export default function App() {
  ...
  useEffect(() => {
    document.title = route.page === 'home' ? 'AURA — Virtual galleries for artists' : route.page === 'create' ? 'Create a gallery | AURA' : route.page === 'demo' ? 'Threshold — Danny Hirsch Arts | AURA' : 'Virtual exhibition | AURA';
  ...
  const page = useMemo(() => { if (route.page === 'create') return template ? <Studio initialTemplate={template}/> : <TemplatePicker onChoose={setTemplate}/>; if (route.page === 'demo') return <Demo/>; if (route.page === 'gallery' && route.id) return <PublishedGallery key={route.id} id={route.id}/>; return <Landing/>; }, [route, template]);
  return <><a className="skip-link" href="#main-content" onClick={(event) => { event.preventDefault(); document.getElementById('main-content')?.focus(); }}>Skip to content</a><div id="main-content" tabIndex={-1}>{page}</div></>;
}
