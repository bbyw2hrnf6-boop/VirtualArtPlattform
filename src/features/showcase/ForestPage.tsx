...
export default function ForestPage(){
  ...
  return <main className="obsidian forest-house">
    ...
    <section ref={stage} className="obsidian__stage" aria-label="Forest Fold House preview" data-flight={flight.status}>
      ...
      {status!=='ready'&&<img className="obsidian__poster" src={`${asset}cover.webp?v=5`} alt="Two stone and oak wings joined by a glass bridge, overlooking a shallow woodland watercourt." fetchPriority="high"/>}
    ...
    <section id="forest-rooms" className="obsidian__collection" aria-labelledby="forest-heading">
      ...
      <div className="forest-house__photos">{forestRooms.map(r=><figure key={r.id}><a href={`${asset}${r.image}.webp?v=5`} target="_blank" rel="noreferrer"><img src={`${asset}${r.image}.webp?v=5`} alt={`${r.name} — rendered from the Forest Fold House model`} loading="lazy" width="1920" height="1080"/></a><figcaption>{r.name}</figcaption></figure>)}</div>
