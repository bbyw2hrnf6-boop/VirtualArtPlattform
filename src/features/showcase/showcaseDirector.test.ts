...
import { HOUSE_FLIGHT, SHOWCASE_STOPS, WORLD_FLIGHTS } from './showcaseFlights';
import { createShowcaseDirector } from './showcaseDirector';
import { createObsidianNavigation, canStand } from './navigation';
import { createPavilionNavigation } from './pavilionNavigation';
...
describe('cinematic rails',()=>{
  for(const [name,flight] of Object.entries({house:HOUSE_FLIGHT,...WORLD_FLIGHTS})){
  ...
  it('reverses through exact world boundaries, including the final frame',()=>{
    expect([0,22/64,42/64,1,41/64,21/64,0].map(p=>worldFrame(p).index)).toEqual([0,1,2,2,1,0,0]);
...
describe('guided showcase visits',()=>{
  ...
  function setup(reduced=false){
    ...
    const director=createShowcaseDirector({camera,navigation:createObsidianNavigation(),stops:SHOWCASE_STOPS.obsidian,flight:HOUSE_FLIGHT,world:WORLD_FLIGHTS.obsidian,reduced:()=>reduced,acquire:()=>{},release,schedule:()=>{},onTour:s=>states.push(s)});
  ...
  it('finishes a flight on time and restores the preflight walking camera',()=>{
    ...
    x.tick(31_000);expect(x.director.active()).toBe(false);expect(x.camera.position.equals(before)).toBe(true);
