...
- Visual wall selection, automatic free-slot placement, click-to-place, direct dragging, and precision sliders.
- An open-roof editing view plus a finished-ceiling preview.
...
- Cinematic gallery introductions, 1.75 m visitor eye height, walk and overview modes, artwork information cards, and click-to-walk navigation.
...
| `npm run check` | Run lint and the complete production build; this is the deployment quality gate. |
...
6. Use the translucent side arrows to rotate around the room.
7. Keep **Open roof** enabled while arranging the exhibition, then use **Preview ceiling** to inspect the finished room.
8. Add and position objects by dragging them or clicking an empty floor location.
9. Optionally run **AI Curator**. Re-running it intentionally produces a new proposal, and its result can be undone during the current editing session.
10. Publish to receive a share link and add the gallery to Discover for ten days.
...
├── services/
│   ├── firebase.ts                 Firebase web-client initialization
│   ├── galleryRepository.ts        Persistence boundary and ten-day lifecycle
...
The current runtime rooms are generated procedurally in `GalleryScene.tsx`; they are not loaded from the three `.blend` files. The Blender files and `blender/create_templates.py` are editable concept/reference sources and currently do not reproduce the latest runtime dimensions or Grand Forum architecture.
