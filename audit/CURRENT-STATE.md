...
The post-story homepage collection includes the locally implemented Obsidian showcase at `/#/showcase/obsidian`. Art exhibitions opens three connected Blender-authored galleries using the owner's eleven supplied AI-generated artworks. It is a read-only bespoke demo, independent of Studio templates and Firebase publications. Entry explicitly loads a desktop/mobile GLB; the artwork directory works without WebGL. Cycles-baked diffuse lighting and room-local floor reflections trade runtime lighting flexibility for presentation quality. Sculpture and Architecture retain their future-showcase states, and the bespoke contact route remains pending. See the [Obsidian production contract](../blender/showcases/obsidian/README.md). No deployment of this showcase has been performed.

The separate lazy showcase intentionally adds 6,000 bytes JS and 2,000 bytes CSS to aggregate release allowances (602,000 / 56,500). Public-entry, largest-lazy-chunk and Studio GLB budgets remain unchanged. The bespoke assets are loaded only on entry, with lower-resolution mobile exports; physical-device qualification remains open.
