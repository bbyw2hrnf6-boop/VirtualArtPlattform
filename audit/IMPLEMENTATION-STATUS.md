...
> Chromium CI timing correction after `11f421d`, 2026-09-10: run `34454116851` and a new Ubuntu reproduction show browser queries outlasting the default five-second assertion, plus successful interactions exhausting a 60-second journey. Functional browser budgets are now centralized at 30 seconds per assertion/action and 180 seconds per journey; CI explicitly selects pinned Chromium/SwiftShader. All twelve tests, full diagnostics, room assets, rendering settings and frontend performance ceilings remain. See [Linux reproduction and verification](./LIEUVA-CI-SMOKE-2026-09-10.md). This supersedes earlier unchanged-timeout notes and does not assert a deployed release.

> CI trace correction after `5b2846e`, 2026-09-10: retained traces from run `34410049211` show overall journey timeouts after successful homepage steps, plus repeated unnecessary rendering in stationary Arrange. Arrange now redraws on actual scene/input changes; focused tests retain both real Studio handoffs and mobile editing/Undo, with unchanged timeout and quality budgets. See [trace evidence and validation](./LIEUVA-EDITORIAL-STORY-2026-09-09.md#ci-trace-correction-after-revision-5b2846e--10-september-2026). Room, material and Blender assets are unchanged; this records a local fix, not remote deployment success.

> CI follow-up after `25eceee`, 2026-09-09: GitHub run `34394176346` still failed during story interactions. The homepage now stops submitting unchanged 3D frames, redraws after material/texture/reflection changes and acknowledges explicit chapter selection immediately. Native scroll/film interpolation remains. Reflection baking repairs invalid HDR samples before filtering; software-rendered interiors remain lit with the same room/material resolutions. Tests distinguish UI state from a rendered, settled frame and exercise a software GPU plus CPU throttling, including a rendered wall-color regression. Failed workflow traces are now retained. Details and local evidence are in [editorial acceptance](./LIEUVA-EDITORIAL-STORY-2026-09-09.md#second-ci-follow-up-after-revision-25eceee); no remote success is asserted before a new owner-triggered run.
