...
Forest desktop delivery is a versioned external glTF package; its relative binary/image dependencies must resolve inside that directory. Mobile remains a GLB. Validate source ceiling/slab separation, non-overlapping wall corners and complete transformed plant bounds before baking. Keep dedicated ceiling charts and 4K exterior transport. Preserve shared vegetation geometry, spatial culling cells and `EXT_mesh_gpu_instancing`; do not flatten instances into duplicate vertex buffers. No delivery file may exceed 100 MiB. With raw exports present, the asset report checks unchanged expanded triangle counts and identical texture payload hashes through packaging.
