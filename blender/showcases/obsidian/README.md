...
on desktop and 1024 px on mobile, with 2×/no MSAA respectively. Two initial bounded frames calibrate the shared viewer before full quality is restored on fast GPUs. Slow frame delivery retains at most 600,000 canvas pixels, a 512 px reflection without MSAA and 4× anisotropy; two later frames above 150 ms also trigger this fallback. Calibration waits asynchronously for actual GPU completion using WebGL fences; fast JavaScript submission and RAF callbacks alone cannot establish GPU speed. Later checks include synchronous render cost plus queued frame latency, excluding idle time. The fence and its polling timer are released on teardown. Regression coverage models both delayed GPU completion and slow synchronous drawing. The completion check adds about 158 bytes gzip to production JavaScript; its aggregate allowance explicitly rises by 256 bytes, with entry, largest-chunk and CSS ceilings fixed. This measured slow-GPU fallback preserves source textures and baked lighting, trading supersampling/reflection sharpness for responsiveness. It uses no CI or user-agent override. This is not live
