...
for room in ['R1','R2','R3']:
    for group,size in [('architecture',2048),('floor',2048),('furniture',1024)]:
        ...
        image.filepath_raw=str(MAPS/f'{o.name}.png');image.file_format='PNG';image.save()
        pending.append((o,image))
        report.append({'mesh':o.name,'size':size,'seconds':round(time.time()-start,2)})
